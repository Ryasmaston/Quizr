...
function TakeQuizPage() {
  ...
  useEffect(() => {
    ...
    async function fetchUser() {
      ...
      try {
        ...
        if (!mounted) return;
        const favs = Array.isArray(body.user?.favourites)
  ...
  const leaderboard = useMemo(() => {
    ...
    attempts.forEach((attempt) => {
      ...
      if (correct > existing.bestCorrect) {
        ...
        if (
          attemptedAt &&
          (!existing.bestAttemptAt || attemptedAt < existing.bestAttemptAt)
        ) {
          existing.bestAttemptAt = attemptedAt;
        }
      }
    ...
    entries.sort((a, b) => {
      ...
      const aTime = a.bestAttemptAt
        ? a.bestAttemptAt.getTime()
        : Number.POSITIVE_INFINITY;
      const bTime = b.bestAttemptAt
        ? b.bestAttemptAt.getTime()
        : Number.POSITIVE_INFINITY;
  ...
  const currentQuestionAnswered = hasAnswered[currentIndex] || false;
  const correctAnswerIds = question.answers
    .filter((answer) => answer.is_correct)
    .map((answer) => answer._id);

  function handleSelect(answerId) {
    if (result || currentQuestionAnswered) return;
    setAnswers((prev) => {
      const updated = [...prev];
  ...
  function getAnswerStyle(answerId) {
    if (!currentQuestionAnswered) {
      return currentSelections.includes(answerId)
    ...
    if (isCorrect) {
      return "bg-emerald-500/30 border-emerald-400 ring-2 ring-emerald-400/70 text-white";
    } else if (isSelected) {
      return "bg-rose-500/30 border-rose-400 ring-2 ring-rose-400/70 text-white";
    } else {
      return "bg-white/10 border-white/20 text-gray-100 opacity-60";
    }
  }
  ...
  async function handleToggleFavourite() {
    ...
    try {
      ...
      setFavouriteIds((prev) =>
        next
          ? prev.filter((itemId) => itemId !== quiz._id)
          : [...prev, quiz._id],
      );
    }
  }
      ...
      <main className="relative max-w-3xl mx-auto px-4 sm:px-6 lg:px-8 py-8 sm:py-12 min-h-full">
        ...
        {phase === "intro" && (
          <div className="bg-white/10 backdrop-blur-lg rounded-3xl border border-white/20 overflow-hidden">
            <div
              className={`flex flex-col sm:flex-row sm:items-center sm:justify-between gap-3 px-6 py-4 sm:px-8 bg-gradient-to-r ${
                  ...
                  <span className="inline-flex h-10 w-10 items-center justify-center rounded-xl bg-white/10">
                    ...
                    >
                      <path
                        ...
                        strokeLinejoin="round"
                        strokeWidth={2}
                        d="M3 10h7l-1.5-1.5m0 0L10 6m-1.5 2.5H21M21 14h-7l1.5 1.5m0 0L14 18m1.5-2.5H3"
                      />
                    </svg>
                  </span>
                  <div className="text-left pl-1">
                ...
                >
                  ...
                  >
                    <path
                      strokeLinecap="round"
                      strokeLinejoin="round"
                      d="M12 3l2.7 5.7 6.3.9-4.6 4.5 1.1 6.3L12 17.9 6.5 20.4l1.1-6.3L3 9.6l6.3-.9L12 3Z"
                    />
                  </svg>
                  <span>
                    {isFavourited
            ...
            <div className="grid gap-3 sm:grid-cols-2">
              {question.answers.map((answer) => {
                return (
                  <button
                    key={answer._id}
                    className={`text-left px-4 py-3 rounded-xl border transition-all ${getAnswerStyle(answer._id)}`}
